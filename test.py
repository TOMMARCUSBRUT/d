...
def handler(signal_received, frame):
    ...
    if current_process().name == "MainProcess":
        pretty_print(get_string("sigint_detected")
                     + Style.NORMAL
                     + Fore.RESET
                     + get_string("goodbye"),
                     "warning")
...
try:
        ...
        #context.bot.send_message(chat_id=update.effective_chat.id,
         #                        text="Xxhash is not installed - this mining algorithm will be disabled")
...
#        context.bot.send_message(chat_id=update.effective_chat.id, text="Cpuinfo is not installed. "
 #                                                                       + "Miner will try to automatically install it "
  #                                                                      + "If it fails, please manually execute "
...
#        context.bot.send_message(chat_id=update.effective_chat.id, text="Pypresence is not installed. "
 #                                                                       + "Miner will try to automatically install it "
  #                                                                      + "If it fails, please manually execute "
...
def mine(update, context):
    ...
    def pretty_print(msg: str = None,
        ...
        with thread_lock():
            ...
            # context.bot.send_message(chat_id=update.effective_chat.id,text=Fore.WHITE + datetime.now().strftime(Style.DIM + "%H:%M:%S ")
    ...
    def share_print(id, type,
        ...
        with thread_lock():
            ...
            context.bot.send_message(chat_id=update.effective_chat.id, text="cpu no"+"="+str(id)+"\n"
                                                                            +"shares accepted"+"="+str(accept)+"/"+str(accept + reject)+"\n"
                                                                            +"shares rejected"+"="+str(reject)+"\n"
                                                                            +"efficency"+"="+str(round(accept / (accept + reject) * 100))+"\n"
                                                                            +"time taken to complte share"+"="+str("%04.1f" % float(computetime))+"\n"
                                                                            +"hashrate"+"="+str(total_hashrate)+"\n"
                                                                            +f"ping ={(int(ping))}ms")
...
updater = Updater(token='1911449860:AAH9rJWND8CTCiHyAZamlfMuBpusVgfUDmo', use_context=True)
